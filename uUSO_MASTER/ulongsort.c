...
#include "watchdog.h"
 ...
 unsigned long mas_copy[ADC_BUF_SIZE];
 unsigned char  mid_cycle_counter=0;
 unsigned long  temp;
...
//#pragma OT(0,Speed) 
 ...
 {
   static unsigned char counter=0,i=0,j=0;
  ...
  while(1) 
  {
	  ...
	  //------------ищем новое измерение-----------------
		...
		{
			counter=0;
			while((!adc_channels[mid_cycle_counter].new_measuring) && (counter!=CHANNEL_NUMBER))
			{
				mid_cycle_counter= (mid_cycle_counter+1);
				if(mid_cycle_counter>=CHANNEL_NUMBER)
				{
					mid_cycle_counter=0;	
				}
				counter++;
